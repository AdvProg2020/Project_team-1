...
public class AuctionsPage {
    ...
    public void setCommodities(Pane root) throws Exception {
        ...
        int i = 0;
        int j = 100;
        dos.writeUTF("send all auctions");
        ArrayList<common.model.commodity.Auction> auctions = yaGson.fromJson(dis.readUTF(), new TypeToken<ArrayList<common.model.commodity.Auction>>() {
        }.getType());
        ArrayList<Commodity> commodities = new ArrayList<>();
        for (Auction auction : auctions) {
            dos.writeUTF("send commodity with id " + auction.getCommodityId());
            commodities.add(yaGson.fromJson(dis.readUTF(), new TypeToken<Commodity>() {
            }.getType()));
        }
        ...
        for (Commodity commodity : commodities) {
            ...
            Auction auctionTmp = null;
            for (Auction auction : auctions) {
                if (auction.getCommodityId() == tmp.getCommodityId()) {
                    View.auctionMenu.setAuction(auction);
                    auctionTmp = auction;
                }
            }
            Auction finalAuctionTmp = auctionTmp;
            imageView.setOnMouseClicked(mouseEvent -> {
                View.auctionMenu.setPreviousMenu(MenuHandler.getInstance().getCurrentMenu());
                View.auctionMenu.setAuction(finalAuctionTmp);
            ...
            Label price = createLabel(i + 300, j + 50, "Price", String.valueOf(auctionTmp.getTopBid()), false);
            Label score = createLabel(i + 300, j + 100, "Bidder", auctionTmp.getTopBidder() == null ? "Nobody" : auctionTmp.getTopBidder(), false);
